...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-[background-color,border-color,color,box-shadow,translate] duration-(--duration-fast) ease-(--ease-out) outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        // Hover DARKENS to `--primary-hover` (brand-hover in light, brand-bright in dark).
        // shadcn's `hover:bg-primary/80` made the most important button on every screen
        // lighter and more transparent under the cursor while the tokens defined a hover
        // step nobody used — the 2026-09-03 baseline measured it.
      ...
      */
      size: {
        ...
        // `lg` is the counter action: the one button a page exists for — «ثبت فاکتور»,
        // «تحویل دستگاه», the auth submit. 48px and the body size, so it is dimensionally
        // distinct from the 40px secondaries beside it; at 44px it was not.
