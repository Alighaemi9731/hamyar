...
### Typography — the pairing, final

Two families and no third. Owner's instruction, 2026-09-05; it supersedes the single-family
choice recorded at the 16.2 gate, and ADR 0020 carries the amendment and the reason.

| role | face | weights | rule |
|---|---|---|---|
| Display / headings | **Estedad** | 700, 800 | `-0.02em` tracking at large sizes; leading 1.1 |
| Body / UI | **Vazirmatn** | 400, 500 (600 max for emphasis) | leading **1.7** prose, **1.5** UI |
| Financial figures | Vazirmatn + `tabular-nums` | — | every money figure, every table column, every invoice line |
| Latin fallback | Inter, then the system stack | 400–600 | see the note below — it never actually downloads |

**Nothing else renders a heading.** Not a `font-semibold` on the sans stack, not a
one-off `font-family`. Two tokens carry it, `--font-display` and `--font-sans`, defined
once in `brand.css`; a hardcoded family anywhere is the bug this section exists to
prevent. There was exactly one when this was written — the Storefront layout declared
Vazirmatn with no `@font-face` behind it, so every forwarded price list rendered in the
system font.

- **Self-hosted, subset, committed.** Five variable `woff2` under `resources/fonts/`,
  regenerated by `bin/subset-fonts` from the sources beside them in `src/`. No CDN: a
  build that fetches fonts is a build that fails the day a CDN does. **505K → 144K, 71%
  saved**; the axis is pinned per family to the weights above, so a weight this table does
  not name is not merely discouraged, it is absent from the file.
- **Preload two files** — `vazirmatn-arabic` and `estedad-arabic`, the two the first
  viewport cannot be painted without. `font-display: swap` everywhere, with
  metric-matched fallback faces so the swap moves nothing: measured **CLS 0.0007** at
  390px on a cold cache at 200 kbps / 150 ms, and no FOIT.
- **Inter is declared and never fetched.** It is subset to the same Latin range Vazirmatn
...
- Scale: 12 / 13 / 15 / 17 / 21 / 28 / 40 / 56 / 72, **with a phone floor of 14px** —
  ...
  handset is not small type, it is unread type. The desktop steps return at `sm`.
- **The landing's display size engages at ≥1280px only.** Below that the hero headline is
  a clamp topping out at 44px; 60px type in a 390px column is a headline that wraps four
  times.
